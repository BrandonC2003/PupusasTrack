...
class ActualizarProductoScreen extends StatefulWidget {
  final CatalogoProductoEntity pupusa;
  const ActualizarProductoScreen({super.key, required this.pupusa});
...
class _ActualizarProductoScreenState extends State<ActualizarProductoScreen> {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => sl<ActualizarProductoBloc>()
        ..add(
          SetInitialValuesEvent(
            id: widget.pupusa.id,
            nombre: widget.pupusa.nombre,
            descripcion: widget.pupusa.descripcion ?? '',
            precio: widget.pupusa.precio,
            disponible: widget.pupusa.disponible,
            descuentos: widget.pupusa.descuentos!
                .map((d) => Descuento(id: '', cantidad: d.cantidad, precio: d.precio))
                .toList(),
          ),
        ),
      child: Scaffold(
        appBar: _buildAppBar(context, 'Actualizar Pupusa'),
        body: BlocListener<ActualizarProductoBloc, ActualizarProductoState>(
          listener: (context, state) {
            if (state.actualizarProductoStatus == ActualizarProductoStatus.failure) {
              ScaffoldMessenger.of(
                context,
              ).showSnackBar(SnackBar(content: Text(state.errorMessage)));
            }

            if (state.actualizarProductoStatus == ActualizarProductoStatus.success) {
              context.pop(true);
            }
          },
          child: BlocBuilder<ActualizarProductoBloc, ActualizarProductoState>(
                      ...
                      child: Column(
                        children: [
                          TextFormField(
                            ...
                            initialValue: widget.pupusa.nombre,
                            onChanged: (value) => context
                                .read<ActualizarProductoBloc>()
                          ...
                          TextFormField(
                            ...
                            initialValue: widget.pupusa.descripcion,
                            onChanged: (value) =>
                                context.read<ActualizarProductoBloc>().add(
                          ...
                          TextFormField(
                            ...
                            initialValue:
                                widget.pupusa.precio.toStringAsFixed(2),
                            onChanged: (value) => context
                                .read<ActualizarProductoBloc>()
  ...
  Widget _buildAgregarMaterialButton(BuildContext context) {
    return BlocBuilder<ActualizarProductoBloc, ActualizarProductoState>(
      builder: (context, state) {
        if (state.actualizarProductoStatus == ActualizarProductoStatus.loading) {
          return Center(
            child: SizedBox(
              width: 24,
              height: 24,
        ...
        return ElevatedButton(
          onPressed: () =>
              context.read<ActualizarProductoBloc>().add((SubmitProductoEvent())),
          child: const Text('Actualizar Pupusa'),
        );
      },
    );
  }
