...
class AgregarMaterialBloc extends Bloc<AgregarMaterialEvent, AgregarMaterialState>{
  ...
  void _onGuardarMaterial(GuardarMaterial event, Emitter<AgregarMaterialState> emit) async {
    ...
    try {
      if (state.nombreStatus != NombreStatus.valid) {
        emit(state.copyWith(status: AgregarMaterialStatus.failure, errorMessage: 'Complete los campos correctamente'));
        emit(state.copyWith(status: AgregarMaterialStatus.initial, errorMessage: ''));
        return;
      }
      ...
      emit(state.copyWith(status: AgregarMaterialStatus.failure, errorMessage: "Ocurrió un error al guardar el material"));
    }
  }
